...
### `SandboxViolationError: Sandbox boundary breached` (v1.3.0+)

Tool code attempted to access the enforcement layer's memory or perform a disallowed operation inside the subprocess sandbox.

**Fix:** Review the tool code. The subprocess sandbox isolates tool execution from the enforcement layer.

### `SensitivityViolationError: Data flow from CONFIDENTIAL to PUBLIC denied` (v1.4.0+)

Sensitivity labels detected data flowing from a higher classification to a lower one.

**Fix:** Ensure data flows respect the label hierarchy: `PUBLIC → INTERNAL → CONFIDENTIAL → RESTRICTED`.

### `StreamingViolation: PII detected in streaming chunk` (v1.11.0+)

The streaming enforcer detected PII in a token chunk during LLM streaming.

**Fix:** The `StreamingRedactor` will auto-redact. If you need to handle this differently, catch `StreamingViolation` and implement custom logic.

### `PluginLoadError: Failed to load plugin` (v1.9.0+)

A plugin could not be loaded by the `PluginManager`.

**Fix:** Ensure the plugin package is installed and implements the correct interface (`GuardPlugin`, `RedactorPlugin`, or `AuditBackendPlugin`).

### `ComplianceError: Compliance report generation failed` (v1.8.0+)

The compliance reporter could not generate a report — usually due to missing audit data.

**Fix:** Ensure audit logging is enabled and the audit trail has data.

### `PolicyServerError: Failed to fetch remote policy` (v1.7.0+)

The `PolicyServerClient` could not retrieve a policy from the remote endpoint.

**Fix:** Check the server URL, HMAC key configuration, and network connectivity.
...
6 built-in regex categories: email, phone, SSN, credit card, IP address, passport. 4 redaction strategies: placeholder, mask, hash, remove. Since v1.4.0, the optional NER backend (`enforcecore[ner]`) adds healthcare, financial, and domain-specific PII detection via Presidio. See the [API Reference](/enforcecore/api-reference#redactor-api) for the full table.

### Why regex instead of spaCy/Presidio by default?

The default regex engine has zero NLP dependencies, is portable, and runs at ~0.1–0.5ms per call. Since v1.4.0, Presidio-based NER detection is available as an optional extra: `pip install enforcecore[ner]`.
...
As of v1.14.0, EnforceCore has **63 public API symbols** — the stable contract following semantic versioning. The original 30 core symbols are augmented with symbols from newer modules (streaming, plugins, compliance, multi-tenant, etc.). See the [API Reference](/enforcecore/api-reference) for the full list.

### Is EnforceCore stable enough for production?

Yes. EnforceCore is **stable** (v1.14.0) with 2,347 tests, 95% coverage, 22 formal invariants, and a 63-symbol public API following semantic versioning.
