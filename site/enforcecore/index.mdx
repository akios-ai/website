...
> **Current release: v1.0.0b3 (beta)** — Production-grade enforcement, frozen API, actively hardened. [Available on PyPI](https://pypi.org/project/enforcecore/).
...
| **Merkle Auditor** | SHA-256 chained audit trail. Every call logged. Tamper detection built in. 4 pluggable backends (JSONL, Null, Callback, Multi). Audit rotation with gzip compression. JSON-safety validation on every entry. |
...
> **Note:** EnforceCore is currently in **beta** (v1.0.0b3). The 30-symbol public API is frozen — no additions or removals until v2.0.0. All 110 original symbols remain importable for backwards compatibility, but the stable contract covers the 30 Tier 1 symbols.
...
| **A3 — Insider** | Write access to code or env vars | Fail-open warnings, append-only Merkle audit, settings logging |
