...
## LangChain Callback Handler (v1.13.0+)

For LangChain applications, EnforceCore provides a callback handler that automatically enforces every LLM call, tool call, and chain step:

```python
from enforcecore.integrations.langchain import EnforceCoreCallbackHandler

# Create handler with your policy
...
result = agent.invoke({"input": "Search for..."}, config={"callbacks": [handler]})
```

The callback handler:
- Enforces policy on every LLM call and tool invocation
- Redacts PII from inputs and outputs automatically
- Records audit entries for every operation
- Tracks cost across the entire chain
- Works with LangGraph, LangChain, and any `langchain_core`-based system

> **Note:** As of v1.14.0, `EnforceCoreCallbackHandler` has been submitted upstream to `langchain-community` for official inclusion.
