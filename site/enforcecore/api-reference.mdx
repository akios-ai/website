...
EnforceCore exposes **63 public API symbols** from the top-level `enforcecore` package — the stable contract. Additional internal symbols remain importable for backwards compatibility but are not covered by the stability guarantee.

> **API Stability:** The 63 public symbols follow semantic versioning as of v1.2.0. Breaking changes only in major releases.

### Public API Symbols (63)
...
| **Audit Stores** (v1.2.0+) | `JsonlAuditStore`, `SqliteAuditStore`, `PostgresAuditStore` |
| **Resource Management** | `ResourceGuard`, `CostTracker`, `KillSwitch`, `RateLimiter` |
| **Subprocess Sandbox** (v1.3.0+) | `SubprocessSandbox`, `SandboxViolationError` |
| **Sensitivity Labels** (v1.4.0+) | `SensitivityEnforcer`, `SensitivityLabel`, `SensitivityViolationError` |
| **NER PII** (v1.4.0+) | `NERRedactor` (requires `enforcecore[ner]`) |
| **Multi-Tenant** (v1.6.0+) | `MultiTenantEnforcer` |
| **Policy Server** (v1.7.0+) | `PolicyServerClient`, `PolicyServerError` |
| **Compliance** (v1.8.0+) | `ComplianceReporter`, `ComplianceError` |
| **Plugins** (v1.9.0+) | `GuardPlugin`, `RedactorPlugin`, `AuditBackendPlugin`, `PluginManager`, `PluginLoadError` |
| **Streaming** (v1.11.0+) | `stream_enforce`, `StreamingRedactor`, `StreamingViolation`, `StreamingContext`, `StreamingResult` |
| **LangChain** (v1.13.0+) | `EnforceCoreCallbackHandler` |
