...
AKIOS provides six core agents:
...
| **webhook** | Notifications (v1.1.0+) | `notify`, `send` |
| **database** | Database queries (v1.1.0+) | `query`, `count`, `execute` |
...
### Agent: webhook (Notifications) (v1.1.0+)

Send webhook notifications to external platforms.

**Actions:**
- `notify` - Send to a named platform (Slack, Teams, Discord, PagerDuty)
...
### Agent: database (Queries) (v1.1.0+)

Read-only database queries with parameterised execution.

**Actions:**
- `query` - Run a SELECT query and return rows
- `count` - Run a COUNT query and return the count
- `execute` - Run an INSERT/UPDATE/DELETE (requires `allow_writes: true`)
...
**✓ Do:**
- Use valid agent names (llm, http, filesystem, tool_executor, webhook, database)
