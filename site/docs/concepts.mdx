...
## EnforceCore
The enforcement engine inside AKIOS has been extracted into an independent open-source framework: [EnforceCore](enforcecore.html). Now in **beta** (v1.0.0b3) with a frozen 30-symbol API, EnforceCore provides the same policy enforcement, PII redaction, Merkle audit, and resource guards — available to any Python agent framework under Apache 2.0.
