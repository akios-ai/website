...
> **Current version: {{version}}.** 6 core agents, 7 LLM providers (including local via Ollama), parallel execution, plugin system, 44 PII patterns, Merkle audit proofs, HTML compliance reports, and ablation benchmarking.
...
- **An API key** from OpenAI, Anthropic, Grok, Mistral, Gemini, or AWS Bedrock (get [free keys below](#step-3-configure-your-api-key-2-minutes)) — or use **Ollama** for free local models
...
- [AWS Bedrock](https://aws.amazon.com/bedrock/) - Claude, Llama, Titan (IAM auth, no API key)
- [Ollama](https://ollama.com/) - Free local models (Llama, Mistral, Gemma — no API key)
