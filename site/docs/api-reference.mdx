...
AI model calls with budget tracking. Supports 7 providers: OpenAI, Anthropic, Grok, Mistral, Gemini, AWS Bedrock (IAM), and Ollama (local).

```python
from akios.core.runtime.agents.llm import LlmAgent

# Create agent (cloud provider)
llm = LlmAgent({
    "provider": "grok",
    "model": "grok-3",
    "api_key": "your-key",
...
Send webhook notifications to external platforms.
...
result = webhook.send(
    ...
    payload={"event": "done", "status": "success"}
)
```

**Available methods:**
- `notify(url, payload, platform=None)` → dict
- `send(url, payload, headers=None)` → dict

**Supported platforms:** Slack, Microsoft Teams, Discord, PagerDuty, generic HTTP POST.

### DatabaseAgent (v1.1.0+)

Read-only database queries with parameterised execution.
...
# Count
count = db.count("SELECT COUNT(*) FROM orders")
print(f"Total orders: {count}")
```

**Available methods:**
- `query(sql, params=None)` → List[dict]
- `count(sql, params=None)` → int
- `execute(sql, params=None)` → dict (requires `allow_writes: true`)

## REST API (v1.0.9+)

AKIOS exposes a FastAPI-based REST API via `akios serve`.

```python
import httpx

# Run a workflow via REST API
response = httpx.post("http://localhost:8000/api/v1/workflow/run", json={
    "workflow": "workflows/process-document.yml",
    "context": {"user_id": "123"}
})
print(response.json())
...
The default regex-based detector covers 44 patterns with >95% accuracy. For additional PII categories (healthcare, financial), use the EnforceCore NER backend (`enforcecore[ner]`).
