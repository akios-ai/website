...
**`audit_retention_days: 0`** (v1.5.0+)
- Automatically prune audit events older than this many days when running `akios audit prune`
- `0` means no automatic pruning
- **Default:** `0`

**`audit_archive_days: 0`** (v1.5.0+)
- Compress and archive audit events older than this many days (`.gz` files)
- `0` means no automatic archiving
- **Default:** `0`

**`use_enforcecore: false`** (v1.2.0+)
- Enable EnforceCore integration for enhanced enforcement
- Requires `pip install akios[enforcecore]`
- When enabled, AKIOS delegates policy evaluation, PII redaction, and audit to EnforceCore
- **Default:** `false`
