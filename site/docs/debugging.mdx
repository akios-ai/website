...
- Bedrock: `anthropic.claude-*`, `meta.llama*`, `amazon.titan-*` (IAM auth)
...
**Allowed agents:** `llm`, `http`, `filesystem`, `tool_executor`, `webhook`, `database`
...
docker run --rm -v $(pwd)/audit:/app/audit akiosai/akios:v{{version}} run workflow.yml

# Or use tmpfs for better performance
docker run --rm --tmpfs /app/audit akiosai/akios:v{{version}} run workflow.yml
```
