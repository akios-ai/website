...
| `agent` | string | Yes | Agent to execute: `llm`, `http`, `filesystem`, `tool_executor`, `webhook`, `database` |
...
- AWS Bedrock: `anthropic.claude-3-5-sonnet-*`, `meta.llama3-*`, `amazon.titan-*`
- Ollama: `llama3.2`, `llama3`, `mistral`, `codellama`, `gemma2`, `phi3` (local models, no API key)
...
- agent: webhook
  ...
  parameters:
    ...
    message: "Workflow completed: ${steps.1.output}"
```
...
**Supported platforms:** `slack` (Slack blocks), `teams` (Teams cards), `discord` (Discord messages), `generic` (plain HTTP POST).

**Security:** PII redaction on messages, HTTPS enforcement, rate limiting (10/min).

### Database Agent (v1.1.0+)
...
**Security:** Parameterized queries only (no raw SQL), DDL always blocked, read-only by default. PII redaction on query results, audit logging of all queries. Max 10,000 rows, 60s timeout.

## Parallel Execution (v1.1.0+)

Run steps concurrently with `parallel:` blocks:
...
steps:
  ...
  - parallel:
    - agent: llm
      action: complete
      parameters:
        prompt: "Summarize: ${steps.1.output}"
    - agent: http
      action: get
      parameters:
    ...
    - agent: llm
      action: complete
      parameters:
        prompt: "Extract entities: ${steps.1.output}"

  - step: 3
    agent: filesystem
    action: write
    parameters:
      path: "data/output/enriched.txt"
      content: "${parallel.output}"
```

**Constraints:** ThreadPoolExecutor with max 4 workers. Thread-safe execution context and cost tracking. Kill switches evaluated between parallel groups. No nested parallel blocks.
...
| Unknown agent | Use one of: `llm`, `http`, `filesystem`, `tool_executor`, `webhook`, `database` |
