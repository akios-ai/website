...
> **Current release: v1.0.0b5 (beta)** — Frozen 30-symbol API, 1,510 tests, 22 formal invariants. See the [full EnforceCore documentation](/enforcecore/) for complete details.
...
graph LR
    ...
    subgraph EC["EnforceCore"]
        ...
        PE["Policy Engine\nYAML + Pydantic"]
        PR["PII Redactor\n5 categories + 11 secret categories"]
        MA["Merkle Auditor\nSHA-256 chain"]
        RG["Resource Guard\ntime / memory / cost"]
    end
...
### PII Redactor
Real-time detection and redaction on inputs and outputs. Configurable strategies: mask, hash, remove, or placeholder. Pure regex engine — no spaCy, no Presidio. 5 PII categories, 11 secret detection categories, Unicode hardening. ~0.028–0.129 ms per call (measured).

### Merkle Auditor
SHA-256 Merkle tree audit trail for every enforced call. JSONL storage with tamper-proof verification. Every entry is cryptographically chained to the previous one. OS-enforced append-only files (`chattr +a` / `chflags uappend`) and hash-only remote witness backends for maximum tamper-evidence.

### Resource Guard
Cross-platform resource limits: time, memory, cost. Linux strict enforcement (RLIMIT_AS), macOS advisory (RLIMIT_RSS), thread timeout on Windows. Shared thread pool with leak detection. Automatic kill-switch when limits are exceeded.
...
EnforceCore is designed for production workloads. Benchmarked on Python 3.13 with 1,000 iterations:

| Component | P50 (ms) | P99 (ms) |
|-----------|----------|----------|
| Policy evaluation | 0.012 | 0.228 |
| PII redaction (short text) | 0.028 | 0.275 |
| PII redaction (~2KB text) | 0.129 | 0.220 |
...
- **Full Documentation:** [akios.ai/enforcecore](/enforcecore/) — Architecture, API reference, formal properties, threat model
