...
**Configures:**
- AI provider (OpenAI, Anthropic, Grok, Mistral, Gemini, AWS Bedrock, Ollama)
- API key with validation (or IAM for Bedrock, or skip for Ollama)
...
- `--report` - Generate HTML security posture report after execution (v1.5.0+)
- `--json-output` - Machine-readable JSON output for CI/CD (v1.0.12+)
...
### `akios serve` — REST API Server (v1.0.9+)

Start a FastAPI-based REST API server for programmatic workflow execution.
...
- `POST /api/v1/workflow/run` — Run a workflow
- `GET /api/v1/workflow/list` — List available workflows
- `POST /api/v1/workflow/validate` — Validate a workflow
- `GET /api/v1/status` — System status
- `GET /api/v1/version` — Version info

### `akios dashboard` — Compliance Dashboard (v1.6.0+)

Launch a local compliance dashboard that reads from your audit trail.
...
### `akios audit migrate` — Audit Migration (v1.5.0+)

Migrate audit events from the primary JSONL ledger to a database.

```bash
# Migrate to SQLite (built-in, no extra deps)
akios audit migrate --backend sqlite --target audit/audit.db

# Migrate to PostgreSQL (requires psycopg2-binary)
akios audit migrate --backend postgresql --target postgresql://user:pass@host/db

# JSON output
akios audit migrate --backend sqlite --target audit/audit.db --json
```

Migration is additive — the source JSONL is preserved.

### `akios audit prune` — Audit Retention (v1.5.0+)

Enforce retention policies on the live JSONL audit ledger.

```bash
# Delete events older than 90 days
akios audit prune --days 90

# Archive events older than 180 days (compressed .gz)
akios audit prune --archive-days 180

# Preview without making changes
akios audit prune --dry-run
```

Reads `audit_retention_days` / `audit_archive_days` from `config.yaml` when no CLI flags provided. Events with unparseable timestamps are always kept.
...
| **Bedrock** | `anthropic.claude-*`, `meta.llama*`, `amazon.titan-*` (via IAM) |
| **Ollama** | `llama3.2`, `llama3`, `mistral`, `codellama`, `gemma2`, `phi3` (local) |
