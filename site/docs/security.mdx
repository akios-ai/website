...
**Ablation flags (v1.0.7):** Benchmark the overhead of individual cage components by selectively disabling them:
...
**PII fail-safe (v1.0.7):** If the PII scanner itself encounters an error during processing, the entire input/output is blocked rather than passed through unredacted. This fail-closed behavior ensures sensitive data never leaks due to scanner failures.
...
**Weighted compliance scoring (v1.0.7):** AKIOS computes a compliance readiness score based on which security components are active.  Each component is weighted by its contribution to a given regulatory framework. Run `akios status --security` to see your current compliance score.
