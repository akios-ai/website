...
**Protected data types (44 patterns across 6 categories):**
- Email addresses, phone numbers (FR/US/UK/DE)
- Social Security numbers (US + French), credit cards
- API keys, passwords, tokens, AWS keys, JWT
- IP addresses (IPv4/IPv6), MAC addresses
- Health insurance IDs, medical records, medication dosages, lab results, diagnosis codes
- Postal addresses, GPS coordinates

> **Note (v1.4.0):** Specialized healthcare identifiers (NPI, DEA, MRN) and financial patterns (IBAN, BIC, routing numbers) were removed from AKIOS OSS to focus on general-purpose detection. Contact us if you need extended PII coverage.

**Applied twice:**
- Before sending to AI providers (inputs)
- After receiving from AI providers (outputs)

**Pluggable PII backend:** AKIOS uses a `PIIDetectorProtocol` interface, allowing custom PII detection implementations. The default regex-based detector covers 44 patterns with >95% accuracy. When EnforceCore is installed (`pip install akios[enforcecore]`), an additional NER-based detection tier (Presidio) and 11-category secret scanner are layered on top.
...
- LLM provider calls (OpenAI, Anthropic, Grok, Mistral, Gemini, AWS Bedrock)
- HTTP agent requests to `allowed_domains`
- Any external call made within the security cage

> **Note:** Ollama runs locally (default `http://localhost:11434`) and is exempt from the HTTPS requirement since traffic never leaves the machine.
